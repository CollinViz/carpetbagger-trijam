...
func _enter_tree() -> void:
	...
	_ref_2d_button.text = "Copy Reference"
...
func _reference_nodes(selection_id: int):
	...
	var updated_code =""
	for node in nodes:
		updated_code+=_reference_node_in_script(node, parent) + "\n"
		
	OS.set_clipboard(updated_code)


func _reference_node_in_script(node: Node, parent: Node)->String:
#	var script: Script = parent.get_script()
#	var code: String = script.get_source_code()
	var code:=""
	return _alter_code(code, node, parent)
...
#			return code
	...
	return _generate_reference(node, parent, code)
