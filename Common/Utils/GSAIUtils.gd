...
static func to_vector2_z(vector: Vector3) -> Vector2:
	return Vector2(vector.x, vector.z)
	
# Returns a vector3 with `vector`'s x and y components and 0 in z.
static func to_vector3(vector: Vector2) -> Vector3:
	return Vector3(vector.x, vector.y, 0)
static func to_vector3_z(vector: Vector2) -> Vector3:
	return Vector3(vector.x, 0,vector.y)

static func positions_round(radius:Vector2,center:Vector2,NumberOfPoints:int)->PoolVector2Array:
	var aRe:PoolVector2Array=[]
